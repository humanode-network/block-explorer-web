...
export default function TransferTable({ moreVariables, noMore }: Props) {
  ...
  const rData =
    data &&
    data.query.transfers.nodes.map((d: TransferData) => ({
      id: d.id,
      block: BlockLink(d.block.id),
      extrinsic:
        d.extrinsicIndex && ExtrinsicLink(`${d.block.id}-${d.extrinsicIndex}`),
  ...
  return (
    <RTable
      {...{
        ...
        loading,
        noMore,
        hasNextPage: data?.query.transfers.pageInfo.hasNextPage,
      }}
    />
  );
}
